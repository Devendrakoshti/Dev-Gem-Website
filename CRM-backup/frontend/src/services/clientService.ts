...
export class ClientService {
    ...
    async restoreClient(id: string): Promise<void> {
        if (USE_DEMO_AUTH) {
            mockStore.updateClient(id, { isDeleted: false }, authService.getCurrentUser()!);
            return;
        }
        await apiClient.post(`/clients/${id}/restore`, {});
    }

    async purgeClient(id: string): Promise<void> {
        if (USE_DEMO_AUTH) {
            const currentUser = authService.getCurrentUser()!;
            // mockStore doesn't have a purge, but we can just use deleteClient if it removes it
            mockStore.deleteClientToTrash(id, currentUser);
            return;
        }
        await apiClient.delete(`/clients/${id}/purge`);
    }
