...
export const ClientsListPage: React.FC<{ archived?: boolean }> = ({ archived = false }) => {
  ...
  const fetchClients = async () => {
    setLoading(true);
    try {
      const data = await clientService.getClients(user, archived ? 'archived' : 'active');
      setClients(data);
    } catch (err: any) {
      ...
      <div className="bg-white rounded-3xl border border-slate-200 shadow-sm overflow-hidden">
        {loading ? (
          <div className="p-20 text-center">
            ...
            <p className="text-slate-500 font-bold">Synchronizing Portfolio...</p>
          </div>
        ) : (
          <>
            {/* Mobile Grid View */}
                  ...
                  <div className="flex items-center justify-between pt-4 border-t border-slate-200">
                    <div className="flex items-center gap-2">
                      <div className="w-6 h-6 rounded-lg bg-indigo-50 border border-indigo-100 flex items-center justify-center text-[10px] font-black text-indigo-600 uppercase">
                        {client.assignedToName?.charAt(0) || 'U'}
                      </div>
