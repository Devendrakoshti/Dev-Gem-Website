...
import { UserRole, ClientStage, ClientStatus, FollowUpType } from '../../../types';
...
export const DashboardPage: React.FC = () => {
  ...
  const [clients, setClients] = useState(mockStore.getCollection('clients'));
  const [employees, setEmployees] = useState(mockStore.getEmployees());
  const [followups, setFollowups] = useState(mockStore.getFollowUps());
  const [billing, setBilling] = useState(mockStore.getCollection('billingItems'));
  const [payments, setPayments] = useState(mockStore.getCollection('paymentsReceived'));
  ...
  // --- Common Calculations ---
  const myClients = useMemo(() => clients.filter(c => c.assignedToId === user.id && !c.isDeleted), [clients, user.id]);
  ...
  const { stats, funnel, myFunnel, leaderboard } = useMemo(() => {
    ...
    // Personal Metrics for Employees
  ...
  }, [clients, billing, payments, employees, myClients]);
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-10">
           ...
           <div className="bg-white rounded-[3rem] border border-slate-200 shadow-sm p-10">
              ...
              <div className="space-y-6">
                 {mockStore.getActivity(user).slice(0, 3).map(log => (
                   <div key={log.id} className="flex gap-4">
                      ...
                      <div>
                         <p className="text-xs font-black text-slate-900 mb-1">{new Date(log.timestamp).toLocaleTimeString()}</p>
    ...
    <div className="space-y-10 animate-in fade-in duration-700 pb-20">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-10">
        ...
        <div className="bg-white rounded-[3rem] border border-slate-200 shadow-sm p-10">
           ...
           <div className="space-y-6">
              {mockStore.getActivity(user).slice(0, 4).map(log => (
                <div key={log.id} className="flex gap-4 relative">
                   ...
                   <div className="flex-1 pb-6">
                      <div className="flex justify-between items-center mb-1">
                         <p className="text-xs font-black text-slate-900">{log.actorName}</p>
                         <span className="text-[9px] font-black text-slate-400 uppercase">{new Date(log.timestamp).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</span>
                      </div>
                      <p className="text-sm text-slate-500 font-medium leading-relaxed">{log.action}</p>
                   </div>
                </div>
