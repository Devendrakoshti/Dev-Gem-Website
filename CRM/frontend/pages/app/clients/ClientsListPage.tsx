...
export const ClientsListPage: React.FC<{ archived?: boolean }> = ({ archived = false }) => {
  ...
  const fetchClients = async () => {
    setLoading(true);
    try {
      const data = await clientService.getClients(archived ? 'archived' : 'active');
      setClients(data);
    } catch (e: any) {
      showToast('Failed to load portfolios: ' + e.message, 'error');
    } finally {
      setLoading(false);
    }
  };
  ...
  useEffect(() => {
    fetchClients();
    // Load employees for transfer dropdown
  ...
  const handleTransfer = async () => {
    ...
    try {
      await clientService.transferClient(transferClient.id, transferTargetId);
      showToast('Client ownership transferred successfully');
      setTransferClient(null);
      setTransferTargetId('');
      fetchClients(); // Refresh list to update owner or remove if no longer assigned
    } catch (err: any) {
  ...
  const filteredAndSortedClients = clients
    .filter(client => {
      const matchesSearch = client.name.toLowerCase().includes(debouncedSearchTerm.toLowerCase()) ||
        ...
        client.companyName.toLowerCase().includes(debouncedSearchTerm.toLowerCase());
              ...
              {filteredAndSortedClients.length === 0 && (
                <tr>
                  <td colSpan={4} className="px-6 py-24 text-center">
                    ...
                    <p className="text-slate-400 font-bold italic text-sm">No clients assigned to your portfolio.</p>
                  </td>
                </tr>
              )}
            </tbody>
