...
export const BackupPage: React.FC = () => {
  ...
  const handleRestore = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    // Using simple confirm for file upload warning as it's a browser-level event interception
    if (window.confirm("CRITICAL WARNING: This will overwrite ALL current database records with the selected backup file. This cannot be undone. Proceed?")) {
    ...
    <div className="space-y-10 animate-in fade-in duration-500">
      <div className="bg-slate-900 rounded-[3rem] p-12 text-white flex flex-col md:flex-row items-center justify-between gap-10 shadow-2xl shadow-indigo-500/10">
        ...
        <button 
          ...
          className="bg-indigo-600 text-white px-10 py-5 rounded-2xl font-bold hover:bg-indigo-500 shadow-xl shadow-indigo-600/20 disabled:opacity-50 flex items-center gap-3 whitespace-nowrap"
        >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-10">
         ...
         <div className="bg-white rounded-3xl border border-slate-200 p-8 shadow-sm flex flex-col items-center text-center h-fit">
            ...
            <p className="text-xs text-slate-400 font-medium mb-6">Restore a Nexus-compliant JSON backup.</p>
            <label className="w-full bg-slate-900 text-white py-4 rounded-xl font-bold text-sm cursor-pointer hover:bg-slate-800 transition-all flex items-center justify-center gap-2">
               <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M7 16a4 4 0 01-.88-7.903A5 5 0 1115.9 6L16 6a5 5 0 011 9.9M15 13l-3-3m0 0l-3 3m3-3v12" /></svg>
               Restore File <input type="file" className="hidden" accept=".json" onChange={handleRestore} />
            </label>
         </div>
      </div>
